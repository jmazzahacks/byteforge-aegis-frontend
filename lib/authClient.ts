...
export function getAuthClientForSite(siteId: string): AuthClient {
  return new AuthClient({
    apiUrl: API_URL,
    siteId,
    tenantApiKey: TENANT_API_KEY,
...
// Resolve the tenant site UUID from the inbound request's Host header by
// looking it up via Aegis's public by-domain endpoint.
export async function resolveSiteIdFromRequest(request: NextRequest): Promise<string | null> {
  const host = request.headers.get('host');
  if (!host) return null;
  const domain = host.split(':')[0];
  const client = getAuthClient();
  const result = await client.getSiteByDomain(domain);
  return result.success ? result.data.uuid : null;
}
